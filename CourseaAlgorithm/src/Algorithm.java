...
///////////////////////////////////
                    ...
                    else if(acceptfindperson) //these are corresponding buttons for a person's information
                    {
                        ...
                        else if(mouseX2 > 120 && mouseX2 < 720 && mouseY2 > 600 && mouseY2 < 650) //use profession
                        {
                            issleep = false;
                            istalk = false;
                            iseat = false;
                            ...
                            getlocation = false;
                            acceptfindperson2 = true;
                            if(personpointer.getTitle() == Person.Title.PROFESSOR)
                            {
                                Professor prof = (Professor)personpointer; //converts Person to Professor (note this only reasonably works since they share the exact same data entries)
                                AskInput3 = prof.Teach(personpointer);
                            }
                            else if(personpointer.getTitle() == Person.Title.STUDENT)
                            {
                                Student stud = (Student)personpointer; //converts Person to Student (note this only reasonably works since they share the exact same data entries)
                                AskInput3 = stud.Study(personpointer);
                            }
                            Whatisusertyping = "";
                        }
                    }
...
//////////////all the below represents the different stages that have buttons in them
        ...
        else if(AcessPerson) //displays for picking people stage
        {
            if(acceptfindperson)//displays for person information stage
            {
                ...
                g.fillRect(Window.getWidth2()/7, 600, 600, 50);
...
///////////             
                ...
                g.drawString("Use Proffesion", Window.getWidth2() * 2/5, 625);
            }
            else
            {
                g.setColor(Color.RED);
